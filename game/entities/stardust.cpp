...
namespace Game
{
    namespace Entities
    {
        Stardust::Stardust(sf::RenderWindow &window, Engine::EntityManager &ecm, sf::Vector2f sfmlPosition, int type) : entity(ecm.createEntity())
        {
            ...
            {
                /*
                float fibo_spacing = SPACING * std::sqrt(i);
                sf::Vector2f offset(
                    fibo_spacing * std::cos(i * FIBO_ANGLE),
                    fibo_spacing * std::sin(i * FIBO_ANGLE));
                ...
                */
                ...
                Game::Components::Physics::createCircle(window, entity->physicsBodyIds.back(), starSfmlPosition, SFML_DIAMETER, DENSITY, FRICTION, RESTITUTION);
                b2Body_SetGravityScale(entity->physicsBodyIds.back(), 0.0f);
        ...
        {
            std::vector<sf::Vector2f> orionsBelt = {{-17.f, 5.f}, {0.f, 0.f}, {17.f, -7.f}};
            std::vector<sf::Vector2f> lyra = {{-37.5f, -22.5f}, {-37.5f, 5.f}, {0.f, 0.f}, {4.f, 33.f}, {12.5f, 60.f}};
